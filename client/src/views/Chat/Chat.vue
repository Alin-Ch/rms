        ...
        <el-container class="center">
            <el-aside width="300px">
                <el-scrollbar v-if="group && group.length > 0" height="55vh">
                    <div class="chatItem" :class="{ 'activeChat': chatItem.id == item.id }"
                        ...
                        <span v-if="item.latest_create_time" style="right: 20px;color: #7f7f7f">
            ...
            <el-main class="main">
                <div class="messageShow">
                    ...
                    <div v-if="chatItem.id && recordContent.length < 1">
                        <el-empty description="暂时还没有消息噢" />
                    </div>
                    <div v-if="recordContent.length < 1">
    ...
    request.get("/sys/message/group/" + userId).then(res => {
        ...
        if (res.code == 200) {
            ...
            if (route.query.userid) {
                ...
                const existingUser = users.some(user => user.id === JSON.parse(localStorage.getItem("house")).owner.id);
                // 如果不存在，则新增用户信息
                if (!existingUser) {
                    users.push(JSON.parse(localStorage.getItem("house")).owner);
                    chatItem.value = JSON.parse(localStorage.getItem("house")).owner;
                    opposite.value = JSON.parse(localStorage.getItem("house")).owner;
            ...
            if (group.value.length > 0) {
                // getChatDetailById(group.value[0])
            }
