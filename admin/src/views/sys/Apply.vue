    ...
    <div>
        <div class="container">
            ...
            <el-table :data="tableData" style="width: 100%" border>
                ...
                <el-table-column prop="user.username" label="发起人" width="180" />
        ...
        <!-- dialog弹出框 -->
        <el-dialog v-model="dialogVisible" :title="dialogMode" width="60%" draggable>
            <el-form label-width="100px" :model="editInfo">
                ...
                <el-form-item label="审批状态">
                    <el-select v-model="editInfo.status" placeholder="审批状态">
...
// 获取表格数据
const getData = () => {
    request.get('/sys/apply/list', { params: params.value }).then((res) => {
...
const handleSubmit = () => {
    if (dialogMode.value == '新增') {
        ...
        request.post("/sys/apply/update", editInfo.value).then((res) => {
            if (res.code == 200) {
...
const handleDelete = (row) => {
    ...
    ElMessageBox.confirm('确定要删除吗？', '提示', {
        ...
        request.post("/sys/apply/delete/" + row.id).then((res) => {
            if (res.code == 200) {
